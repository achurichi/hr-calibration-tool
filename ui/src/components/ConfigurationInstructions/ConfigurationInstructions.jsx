...
const ConfigurationInstructions = observer(({ className, description, images, onScreenChange, title }) => {
  ...
  const imagesBase64 = images.map((id) => descriptionStore.referenceImages.get(id)?.base64).filter((i) => i);

  const showCarousel = !!images.length;
  const loadingImages = images.length !== imagesBase64.length;

  return (
    <div className={classNames('configuration-instructions-container', className)}>
      ...
      <div className="content">
        ...
        {showCarousel && (
          <div className="images">
            ...
            {!loadingImages && <Carousel images={imagesBase64} onScreenChange={onScreenChange} />}
          </div>
        )}
      </div>
    </div>
