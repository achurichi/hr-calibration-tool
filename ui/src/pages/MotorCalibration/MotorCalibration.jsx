...
const MotorCalibration = observer(() => {
  const { configurationStore, filtersStore, robotStore, requestStore } = rootStore;
  const { configure: configurableMotors, add: addableMotors } = useConfigurableItems(DESCRIPTION_TYPES.MOTORS);
  ...
  useEffect(() => {
    ...
    if (searchFilter) {
      motors = motors.filter(
        ({ name, motorId }) =>
          name.toLowerCase().includes(searchFilter.toLowerCase()) || (motorId && `${motorId}`.includes(searchFilter))
      );
    }
  ...
  const onAddMotors = async (motorsMap) => {
    ...
    if (success) {
      await configurationStore.getOrFetchAssemblyConfigurations(DESCRIPTION_TYPES.MOTORS, true);
  ...
  const onDeleteMotor = async () => {
    const { success } = await callWithNotification(
      () => configurationStore.deleteMotor(motorToDelete.assembly, motorToDelete.id),
    ...
    return {
      ...
      action: (
        <div className={styles['action-container']}>
          <EditIconField redirect={`${PATHS.MOTOR_CONFIGURE}/${id}`} tooltipContent="Edit configuration" />
          <ClickableIcon
            color="var(--danger)"
            Icon={BsTrash}
    ...
    <div className={styles.container}>
      <div className={styles['internal-container']}>
        ...
        <ConfirmationModal
          ...
          disabled={requestStore.isLoading(MOTORS_CONFIGURATIONS_REQUESTS.DELETE_ITEM)}
          message={
            <div>
