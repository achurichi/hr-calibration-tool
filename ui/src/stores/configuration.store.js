...
class ConfigurationStore {
  ...
  getItem(id) {
    for (const configuration of this.configurations.values()) {
      const items = configuration instanceof MotorsConfiguration ? configuration.motors : configuration.animations;
  ...
  getNameKey(descriptionType) {
    return descriptionType === DESCRIPTION_TYPES.MOTORS ? 'motorName' : 'animationName';
  }

  async fetchConfiguration(descriptionType, descriptionName, assembly) {
    const data = await this.rootStore.api.calibrationTool.configurations[descriptionType].getByDescriptionAndAssembly(
      descriptionName,
      assembly
    );
  ...
  async getOrFetchConfiguration(descriptionType, descriptionName, assembly) {
    ...
    return configuration || (await this.fetchConfiguration(descriptionType, descriptionName, assembly));
  }
  ...
  async getOrFetchAssemblyConfigurations(descriptionType, forceFetch = false) {
    const assemblyEntries = this.rootStore.robotStore.getAssemblyEntries();
    const configurationPromises = assemblyEntries.map(([assembly, descriptionName]) =>
      forceFetch
        ? this.fetchConfiguration(descriptionType, descriptionName, assembly)
        : this.getOrFetchConfiguration(descriptionType, descriptionName, assembly)
    );
    return await Promise.all(configurationPromises);
  }

  async saveItem(descriptionType, descriptionName, assembly, itemConfiguration) {
    const data = await this.rootStore.api.calibrationTool.configurations[descriptionType].saveItem(
      descriptionName,
      assembly,
      itemConfiguration
    );
  ...
  async addMotors(motorsMap) {
    await this.rootStore.api.calibrationTool.configurations.motors.addItems(motorsMap);
  }

  async deleteMotor(assembly, motorId) {
    const data = await this.rootStore.api.calibrationTool.configurations.motors.deleteItem(assembly, motorId);
    return this._saveConfiguration(DESCRIPTION_TYPES.MOTORS, data);
  }

  async createConfigurations(configurationDescriptionMap) {
    await this.rootStore.api.calibrationTool.configurations.createMany(configurationDescriptionMap);
  }
  ...
  _saveConfiguration(descriptionType, data) {
    ...
    const configuration =
      descriptionType === DESCRIPTION_TYPES.MOTORS ? new MotorsConfiguration(data) : new AnimationsConfiguration(data);
