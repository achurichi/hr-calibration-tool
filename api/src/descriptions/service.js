...
const create = async function (name) {
  const motorsCollection = await mongoDBClient.getCollection(COLLECTIONS.MOTORS_DESCRIPTION);
  const animationsCollection = await mongoDBClient.getCollection(COLLECTIONS.ANIMATIONS_DESCRIPTION);
  let motorsNames;
  let animationsNames;
...
const deleteByName = async function (name) {
  const motorsDescription = await findByName(name, COLLECTIONS.MOTORS_DESCRIPTION);
  const animationsDescription = await findByName(name, COLLECTIONS.ANIMATIONS_DESCRIPTION);
...
const namesByAssembly = async function (assemblies) {
  ...
  try {
    ...
    logErrorAndThrow(err.stack, `Could not get description names for assemblies`);
  }
...
const allDescriptionNames = async function () {
  const collection = await mongoDBClient.getCollection(COLLECTIONS.MOTORS_DESCRIPTION);
...
const saveItem = async function (descriptionName, item, collectionName) {
  ...
  const listProp = collectionName === COLLECTIONS.MOTORS_DESCRIPTION ? 'motors' : 'animations';
  ...
  if (collectionName === COLLECTIONS.ANIMATIONS_DESCRIPTION) {
    ...
    item.motions = item.motions.map((m) => (m.id ? m : { id: new ObjectId().toString(), ...m }));
  }
  ...
  try {
    const collection = await mongoDBClient.getCollection(collectionName);
    await collection.updateOne({ name: descriptionName }, { $set: { ...description, [listProp]: items } });
  } catch (err) {
  ...
  try {
    if (oldItem) {
      if (collectionName === COLLECTIONS.MOTORS_DESCRIPTION) {
        ...
        await Promise.all([deleteFn('neutralPosition'), deleteFn('minPosition'), deleteFn('maxPosition')]);
      } else {
