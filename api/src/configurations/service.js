...
const createMany = async function (items) {
	...
	const motorsConfigCollection = await mongoDBClient.getCollection(
		COLLECTIONS.MOTORS_CONFIGURATION
	)
	const animationsConfigCollection = await mongoDBClient.getCollection(
	...
	try {
		motorsConfigAssemblies = (
			await motorsConfigCollection.find({}).toArray()
		).map(({ assembly }) => assembly)
		animationsConfigAssemblies = (
			await animationsConfigCollection.find({}).toArray()
		).map(({ assembly }) => assembly)
	} catch (err) {
	...
	assemblies.forEach((assembly) => {
		if (
			motorsConfigAssemblies.includes(assembly) ||
			animationsConfigAssemblies.includes(assembly)
		) {
	...
	if (existingAssemblies.length) {
		...
		throw new Error(
			`The following assemblies already have a configuration: ${repeated}`
		)
	}
 ...
 */
const generateDefaultConfiguration = async function ([
	assembly,
	descriptionName,
]) {
	const motorsDescription = await descriptionsService.findByName(
		descriptionName,
