...
const addItems = async function (motorsMap) {
	const configCollection = await mongoDBClient.getCollection(
		COLLECTIONS.MOTORS_CONFIGURATION
	)
	const descCollection = await mongoDBClient.getCollection(
	...
	for (const assembly in motorsMap) {
		...
		try {
			...
			logErrorAndThrow(
				err.stack,
				'Error occurred while getting description and configurations'
			)
		}
		...
		try {
			await configCollection.updateOne(
				{ assembly },
				{ $set: { ...configuration, motors } }
			)
		} catch (err) {
			logErrorAndThrow(
				err.stack,
				'Error occurred while updating motors configuration'
			)
		}
	}
}
...
const deleteItem = async function (assembly, motorId) {
	...
	try {
		...
		logErrorAndThrow(
			err.stack,
			'Error occurred while updating motor configuration'
		)
	}
	...
	try {
		...
		logErrorAndThrow(
			err.stack,
			'Error occurred while getting motor configuration'
		)
	}
}
