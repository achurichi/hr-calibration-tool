...
class MongoDBClient {
  ...
  constructor() {
    if (MongoDBClient.instance) {
      throw new Error('Use MongoDBClient.getInstance() to get the single instance of this class.');
    }
  ...
  async getCollection(collectionName, throwMessageOnError = 'Error accessing database') {
    let db;
    try {
      db = await this.getDB();
    } catch (err) {
